...
namespace BarcodeSplitManage
{
	...
	/*public class PDFSplitter
	{
		...
		public PDFSplitter(MainForm parent, BarcodeOptions options, DirectoryData dirdata, int listIndex)
		{
			_parent = parent;
		...
		public void Run()
		{
			...
			SendMsgEvent(this, new PDFSplitterEventArgs(1, "Running"));
			...
			while (_runable)
			{
				...
				if (Directory.Exists(_directoryData.FolderWatched))
				{
					...
					foreach (string fileName in fileEntries)
					{
						...
						SendMsgEvent(this, new PDFSplitterEventArgs(1, "Processing"));
			...
			_runningState = ServiceState.Ready;
			SendMsgEvent(this, new PDFSplitterEventArgs(1, "Stopped"));
		}
		...
		public void Stop()
		{
			...
			if (_runningState != ServiceState.Ready)
			{
				...
				SendMsgEvent(this, new PDFSplitterEventArgs(1, "Stopping"));
			}
		}
		...
		private bool Split(string srcFileName)
		{
			...
			{
				SendMsgEvent(this, new PDFSplitterEventArgs(2, string.Format("Load error: {0} no exist", srcFileName), TraceEventType.Error));
				return false;
			}
			...
			try
			{
				...
				SendMsgEvent(this, new PDFSplitterEventArgs(2, string.Format("Load success: {0} has {1} pages", srcFileName, pdfDoc.Pages.Count), TraceEventType.Information));
			}
			catch (Exception ex)
			{
				SendMsgEvent(this, new PDFSplitterEventArgs(2, string.Format("Load error: {0}", ex.Message), TraceEventType.Error));
			...
			for (int i = 1; i <= pdfDoc.Pages.Count; i++)
			{
				...
				{
					SendMsgEvent(this, new PDFSplitterEventArgs(2, string.Format("Detect barcodes error in pdf: {0}, page: {1}, {2}", srcFileName, i, ex.Message), TraceEventType.Error));
				}
			}
			...
			if (errorData.Count > 0)
			{
				...
				SendMsgEvent(this, new PDFSplitterEventArgs(2, string.Format("Detect two or more barcodes in pdf: {0}, page(s): {1}", srcFileName, errPages), TraceEventType.Error));
			}
			else if (findedData.Count <= 0)
			{
				dstFileName = Path.Combine(_directoryData.FolderError, Path.GetFileName(srcFileName));
				SendMsgEvent(this, new PDFSplitterEventArgs(2, string.Format("No detect barcodes: {0}", srcFileName), TraceEventType.Error));
			}
			else
			{
				...
				foreach (var fData in findedData)
				{
					...
					try
					{
						...
						SendMsgEvent(this, new PDFSplitterEventArgs(2, string.Format("Extract pdf success: {0}, {1} ~ {2} pages", splitName, pageStart, pageEnd), TraceEventType.Debug));
					}
					catch (Exception ex)
					{
						SendMsgEvent(this, new PDFSplitterEventArgs(2, string.Format("PDF split error, {0} : {1} ~ {2} pages, {3}", splitName, pageStart, pageEnd, ex.Message), TraceEventType.Error));
						errorFlag = true;
						break;
					}
				...
				if (!errorFlag)
				{
					...
					SendMsgEvent(this, new PDFSplitterEventArgs(2, string.Format("Split success: {0}", srcFileName), TraceEventType.Information));
				}
				else
				{
					dstFileName = Path.Combine(_directoryData.FolderError, Path.GetFileName(srcFileName));
		...
		private string GetNewPdfFileName(string pdfFileName, BarcodeData barcodes, int pageStart, int pageEnd)
		{
			...
			string timestamp = DateTime.Now.ToString("MM-dd-yyyy H-mm");
		...
		private bool FileCopy(string srcFileName, string dstFileName, bool overwrite = true)
		{
			...
			{
				SendMsgEvent(this, new PDFSplitterEventArgs(2, string.Format("PDF file copying  error, {0} -> {1} : {2}", srcFileName, dstFileName, ex.Message), TraceEventType.Error));
				return false;
			}
		...
		private BarcodeData[] ReadBarcode(RasterImage image)
		{
			...
			{
				SendMsgEvent(this, new PDFSplitterEventArgs(2, string.Format("Detect barcodes error: {0}", ex.Message), TraceEventType.Error));
			}
